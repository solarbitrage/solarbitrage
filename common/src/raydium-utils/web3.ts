...
import { ASSOCIATED_TOKEN_PROGRAM_ID, SPL_ACCOUNT_LAYOUT, SPL_MINT_LAYOUT, SYSTEM_PROGRAM_ID, TOKEN_PROGRAM_ID, WSOL } from '@raydium-io/raydium-sdk'
...
export async function createAtaSolIfNotExistAndWrap(
  ...
  const mint = new PublicKey(WSOL.mint)
  // @ts-ignore without ts ignore, yarn build will failed
  const ata = await Token.getAssociatedTokenAddress(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, mint, owner, true)
  if (!publicKey) {
