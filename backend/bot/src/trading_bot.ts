...
const WALLET_KEY_PATH =
  ...
  "/Users/noelb/my-solana-wallet/wallet-keypair.json";
...
const getNewConnection = useConnection(false, {
  ...
  fetch: (url, opts) =>
    fetchWithTimeout(fetch, url, { ...opts, timeout: 3000 }),
});
...
orcaListeners.map(
  ([pool, _]) => (poolAddrToOrcaPool[pool.address.toBase58()] = pool)
);
...
async function main() {
  ...
  const loop = () => {
    ...
    const middleTokenToRouteMap =
      getMiddleTokenToRoutesMap(middleTokenToPoolMap);
    ...
    console.table(
      Object.keys(pool_to_slippage_map).map((poolId) => ({
        Pool: poolId,
        "Buy Rate Slippage (%)": (
          pool_to_slippage_map[poolId][0] * 100
        ).toFixed(4),
        "Sell Rate Slippage (%)": (
          pool_to_slippage_map[poolId][1] * 100
        ).toFixed(4),
      }))
    );
  ...
  const waitForValidTokens = new Promise((resolve, _) => {
    gotValidTokens = resolve;
  });
...
async function calculate_trade({ route, estimatedProfit }, index) {
  ...
  const interval = setInterval(() => {
    console.log(
      `waiting on ${route[0].pool_id} -> ${route[1].pool_id}; time elapsed: ${
  ...
  try {
    await arbitrage(
      route,
      usdc,
      usdc + usdc * estimatedProfit,
      estimatedProfit <= THRESHOLD
    );
  } catch (e) {
    console.error(e);
  }
...
const arbitrage = async (
  ...
  _expected_usdc,
  shouldSkipSwap?: boolean
) => {
  const current_pool_to_slippage = JSON.parse(
    JSON.stringify(pool_to_slippage_map)
  );
  let transactionId = "";
  const profitMsg = {
  ...
  try {
    for (const [i, pool] of route.entries()) {
      ...
      const newTokenAmt = i === 0 ? beforeAmt * pool.buy.rate : fromCoinAmount;
      ...
      if (provider === "RAYDIUM") {
        const fromToken =
          fromTokenStr === "SOL"
            ? NATIVE_SOL
            : MAINNET_SPL_TOKENS[fromTokenStr];
        const toToken =
          toTokenStr === "SOL" ? NATIVE_SOL : MAINNET_SPL_TOKENS[toTokenStr];
        ...
        afterSwapPromises.push(
          (async () => {
            const poolKeys = poolKeysMap[_pool_addr];
            const connection = getNewConnection();
            // im sorry
            const _fromToken =
              fromToken.mint === NATIVE_SOL.mint ? WSOL : fromToken;
            const _toToken = toToken.mint === NATIVE_SOL.mint ? WSOL : toToken;

            const amountOut = RaydiumRateFuncs.getRate(
              poolKeys,
              await Liquidity.fetchInfo({ connection, poolKeys }),
              _fromToken,
              _toToken,
              _beforeAmt
            );
            const parsedAmountOut =
              ...
              (1 - ADDITIONAL_SLIPPAGE);

            if (
              parsedAmountOut < newTokenAmt &&
              (_i != 0 ||
                parsedAmountOut * route[_i + 1].sell.rate <
                  newTokenAmt * route[_i + 1].sell.rate)
            ) {
              const slippageShouldBe = Math.max(
                0,
                slippage + (1 - parsedAmountOut / newTokenAmt)
              );
              console.warn(
                `POOL_ID{${pool.pool_id}}[${_i}]: SLIPPAGE_WARNING: ${parsedAmountOut} < ${newTokenAmt} which results in a unprofitable trade (trading on RAYDIUM, slippage should maybe be ${slippageShouldBe})`
              );
              ...
              console.warn(
                `POOL_ID{${pool.pool_id}}[${_i}]: SLIPPAGE_WARNING: ${parsedAmountOut} > ${newTokenAmt} which means slippage might be too high (trading on RAYDIUM, slippage should maybe be ${slippageShouldBe})`
              );
              pool_to_slippage_map[_pool_id][_i] = slippageShouldBe;
            }
          })().catch((e: Error) => {
            console.error(`ERR: POOL_ID{${_pool_id}}[${_i}]:`, e);
          })
        );
        ...
        const currentPool = new OrcaPoolImpl(
          connection,
          Network.MAINNET,
          poolParam
        );
        ...
        afterSwapPromises.push(
          (async () => {
            ...
            const quote = await currentPool.getQuote(
              fromToken,
              new Decimal(_beforeAmt)
            );
            const parsedAmountOut =
              quote.getExpectedOutputAmount().toNumber() *
              (1 - ADDITIONAL_SLIPPAGE);
            ...
            if (
              ...
              const slippageShouldBe =
                slippage + (1 - parsedAmountOut / newTokenAmt);
              console.warn(
                `POOL_ID{${pool.pool_id}}[${_i}]: SLIPPAGE_WARNING: ${parsedAmountOut} < ${newTokenAmt} which results in a unprofitable trade (trading on ORCA, slippage should maybe be ${slippageShouldBe})`
              );
              pool_to_slippage_map[_pool_id][_i] = slippageShouldBe;
            } else if (parsedAmountOut > newTokenAmt) {
              const slippageShouldBe =
                slippage + (1 - parsedAmountOut / newTokenAmt);
              console.warn(
                `POOL_ID{${pool.pool_id}}[${_i}]: SLIPPAGE_WARNING: ${parsedAmountOut} > ${newTokenAmt} which means slippage might be too high (trading on ORCA, slippage should maybe be ${slippageShouldBe})`
              );
              pool_to_slippage_map[_pool_id][_i] = slippageShouldBe;
            }
          })().catch((e: Error) => {
            console.error(`ERR: POOL_ID{${_pool_id}}[${_i}]:`, e.message);
          })
        );
        ...
        if (!shouldSkipSwap) {
          const { transactionPayload } = await orcaSwap(
            ...
            new PublicKey(
              tokenAccounts[fromToken.mint.toBase58()]?.tokenAccountAddress
            ),
            new PublicKey(
              tokenAccounts[toToken.mint.toBase58()]?.tokenAccountAddress
            )
          );
    ...
    if (!shouldSkipSwap) {
      const beforeParsedInfo =
        tokenAccounts[MAINNET_SPL_TOKENS["USDC"].mint]?.parsedInfo;
      ...
      const parsedInfo =
        afterTokenAccounts[MAINNET_SPL_TOKENS["USDC"].mint]?.parsedInfo;
...
function getMiddleTokenToPoolMap(mainToken: string) {
  const poolsWithUSDC = Object.keys(local_database)
    .filter(function (amm) {
      ...
      const mainTokenExists =
        firstCurrency == mainToken || secondCurrency == mainToken;
...
function getMiddleTokenToRoutesMap(middleTokenToPoolMap: any) {
  ...
  for (const middleTokenName of Object.keys(middleTokenToPoolMap)) {
    ...
    for (let x = 0; x < middleTokenToPoolMap[middleTokenName].length; x++) {
      for (
        let y = x + 1;
        y < middleTokenToPoolMap[middleTokenName].length;
        y++
      ) {
        ...
        let estimatedProfits = {
          "a then b":
            1 *
              a.buy.rate *
              (1 - pool_to_slippage_map[a.pool_id][0]) *
              b.sell.rate *
              (1 - pool_to_slippage_map[b.pool_id][1]) -
            1,
          "b then a":
            1 *
              b.buy.rate *
              (1 - pool_to_slippage_map[b.pool_id][0]) *
              a.sell.rate *
              (1 - pool_to_slippage_map[a.pool_id][1]) -
            1,
        };

        if (estimatedProfits["a then b"] > estimatedProfits["b then a"]) {
          middleTokenToRouteMap[middleTokenName].push({
            route: [a, b],
            estimatedProfit: estimatedProfits["a then b"],
          });
        } else {
          middleTokenToRouteMap[middleTokenName].push({
            route: [b, a],
            estimatedProfit: estimatedProfits["b then a"],
          });
        }
      }
    }

    middleTokenToRouteMap[middleTokenName].sort(
...
async function getTokenAccounts() {
  const accounts = await mainConnection.getParsedTokenAccountsByOwner(
    owner.publicKey,
